...
| `js/sprites.js` | Side-view creature art built from genes (limbs, tail, head, coat, armour, spines, wings, jaws) for the fictional worlds; the sprite cache and Codex portraits, which draw catalog species from their templates |
| `js/spritetweaks.js` | P3-ART: each catalog species' sprite tweak set (template, proportions, parts, region colours, markings, pose) derived from its catalog entry, the look-alike distance and per-catalog separation. No canvas; Node loads it |
| `js/templates.js`, `js/templates-more.js` | P3-ART: the hand-drawn layered templates, one per taxon group (quadrupeds, bats, marine mammals, birds; reptiles, amphibians, fish, insects, spiders, crustaceans, snails, worms, plants) |
| `docs/art-style-guide.md` | The species art style guide: palette, outline, the unit frame and anchor points, how templates and tweaks work, adding a template, the taxon groups and their templates |
| `tools/check-sprites.js` | The look-alike check: every built catalog's species compared in a feature space of what's visible (template, parts, proportions, marks, CIELAB region colours); exits 1 on an unresolved look-alike: `node tools/check-sprites.js [code ...] [--verbose]` |
| `tools/sprite-gallery.html` | Dev page: every species of a catalog drawn from its template, grouped by template (`/tools/sprite-gallery.html?c=9.4.6&t=canid,felid&sheet=1`) |
...
**Open:** there's no Hawaiʻi catalog (species sprites are now built, see P3-ART below). With P3-M5's regional pool, cast survival after 3 hands-off rounds is about 81% on 9.3 *Rewilding the ranch* (was about 45% after 5) and 94–98% on the Edwards Plateau (was about 70%).
...
**P3-ART Species sprites: built.** In `js/spritetweaks.js`, `js/templates.js` and `js/templates-more.js`; see `docs/art-style-guide.md`.

- **Templates:** 41 hand-drawn, layered templates cover every taxon group in the catalogs: canid, felid, cervid, bovid (with pigs), ursid, mustelid, rodent (with shrews, moles and the opossum), lagomorph (with pikas), bat, marine mammal; songbird, wader, raptor, waterfowl, gamebird, woodpecker; snake, lizard, salamander, turtle, frog; fish; butterfly, dragonfly, bee (with wasps), ant, beetle, bug, grasshopper, fly, spider (with scorpions), crustacean, snail, worm; forb, grass, shrub, tree, succulent, vine, aquatic. Each is built from parts attached at anchor points on a shared skeleton (quadruped, bird, sprawl, fish, insect…), with recolourable regions and markings clipped to them.
- **Tweak sets** are derived deterministically from the catalog entry: the family's and genus's look, colour and pattern words in the common name ("Red-tailed", "Black-capped", "Yellowlegs", "Sevenspotted", "Four-lined", "Collared"), mass (size class, body depth, antler size) and a small per-species jitter. They cover proportions, variant parts (horns, antlers, crests, bills, tails, wing shapes, flower heads, crowns), region colours, markings and pose. A catalog entry can override any of it with a `tweaks` field. Catalog definitions carry the tweak set into `sp.meta.tweaks` (and onto producers), so saves keep it.
- **Drawing:** the map's close-up sprites and the Codex portrait draw catalog species from their template; sprites are cached per species and size bucket, so tweaks cost nothing per tick. The fictional worlds keep the genome-driven creatures, and unidentified species stay grey icons.
- **Look-alikes:** species of one template in one catalog are compared by what's visible (template, parts, proportions, markings and each region's CIELAB colour, weighted by area). Pairs closer than 0.5, or 0.7 for candidates for the same slot, are separated deterministically by nudging the later species (a marking first, then proportions, parts, lightness, marking colour, hue).

Last `check-sprites.js` run: 100% of the 5,801 catalog entries (plus domestic cattle) have a real template, none fall back. 1,903 look-alike pairs were found before separation across the 18 catalogs (77–178 per catalog) and all were separated; no look-alikes remain. Coyote, red fox and gray fox are 1.4–2.4 apart before any separation.
